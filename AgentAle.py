...
class AgentAle:
    ...
    def updateGP(self,c):
        ...
        x1=np.array([self.prevBids.T[c,:],self.prevBudgets.T[c,:]])
        x1=np.atleast_2d(x1).T
        x2= np.array([self.prevBids.T[c,:],self.prevCosts.T[c,:]])
        ...
        y1=self.prevClicks.T[c,:].ravel()
        y2 = self.prevClicks.T[c,:].ravel()
        y = np.append(y1,y2)
