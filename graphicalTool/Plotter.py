...
class Plotter:
    ...
    def plotClicksAgentBid(self,gpIndex):
        ...
        observedOutput = self.agent.divideFloat(self.agent.prevClicks[:, gpIndex]*24.0,self.agent.prevHours[:,gpIndex])
        plt.figure(gpIndex)
        plt.plot(observedInput, observedOutput, 'r.', markersize=10, label=u'Observations')
    ...
    def plotCostsAgentBid(self, gpIndex):
        ...
        observedInput = self.agent.prevBids[:, gpIndex]
        observedOutput = self.agent.divideFloat(self.agent.prevCosts[:, gpIndex] * 24.0, self.agent.prevHours[:, gpIndex])
